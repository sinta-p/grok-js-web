...
function GrokCollection () {
  ...
  const resolveSubPatterns = pattern => {
    ...
    subPatterns.forEach(matched => {
      ...
      if (!subPattern) {
        debug.error('Error: pattern "' + subPatternName + '" not found!');
        return;
      }
  ...
  t.createPattern = (expression, id) => {
    ...
    if (patterns.has(id)) {
      debug.error('Error: pattern with id %s already exists', id);
    } else {
...
module.exports = {
  loadDefault (loadModules, callback) {
